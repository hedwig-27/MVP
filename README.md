...
- 训练评估：训练阶段优化 NRMSE + 正则；评估阶段在 ID/OOD 上做一步预测，并输出图表与路由使用统计。
...
2) 评估阶段：读取最优模型，对 ID/OOD 测试集做一步预测，并生成评估图。
...
python eval/eval_suite.py --config configs/primitive.yaml --model outputs/latest_primitive/primitive_model.pt
```

## 训练/评估流程说明

- 训练阶段：以 **NRMSE** 为主损失，可叠加 entropy/diversity/load-balance 正则；支持 top-k warmup；保存 Val 最优模型。
...
## 已实现功能
- 数据加载：支持 PDEBench tensor/multi 与 group-per-sample H5；空间/时间下采样；可拼接网格坐标；`sample_ratio` 采样；训练集统计归一化并缓存。
...
- 原语算子：FNO1D 或 CNN 作为匿名原语；输出 delta；聚合器支持 linear 与 MLP（权重+delta）；共享基础算子（小型 FNO）作为“额外原语”参与组合器并赋予权重，且在 top-k 中恒被选中（always-on，load-balance 默认忽略 base）。
- 训练流程：NRMSE + entropy/diversity/load-balance 正则；top-k warmup；三阶段训练（base/residual/joint）；相对指标保存最优模型（可由配置固定）。
- 评估流程：ID/OOD 一步预测；输出 JSON/图表；统计路由使用频次。
...
## 版本控制
- 每次训练生成 `outputs/primitive_YYYYMMDD_HHMMSS/`。
- `outputs/latest_primitive` 软链接指向最近一次训练结果。
- README 统一记录版本与评估结果，并注明对应输出目录。
...
- 输出对应 `outputs/primitive_20260206_140710`。
- 结果：Train 0.018559 / Val 0.034685；ID avg 0.023472；OOD-param avg 0.365772；OOD-eq avg 0.125037。
- 分析：ID 有所退化（burgers、diff_sorp 上升），但 OOD 明显提升；说明当前缩放/权重更偏泛化，需再调低 `loss_scale_max` 或回调 diff_sorp 权重。
### 0205
- 删除所有 rollout 训练/评估相关流程与记录。
- 保存最优模型改为基于相对历史最好值（可由配置固定，关闭自动更新）。

- 输出对应 `outputs/primitive_20260205_110416`、`outputs/primitive_20260206_140710`。
- 归一化对照实验（自动套件）：
...
- 新增 term library 监督与 residual/router 组合训练；base 在 residual 阶段保持开启但冻结。
- 训练/评估配置见 `configs/primitive.yaml`，输出对应 `outputs/primitive_20260203_142150`。
...
- 训练设置：epochs=50，topk_warmup_epochs=5。
- time_downsample=5。

## 结果记录
**记录说明**
- 所有数值均为 **NRMSE**。
- 当前记录 0118/0127/0203/0204/0205/0206；对应输出目录分别为 `outputs/primitive_20260118_202629`、`outputs/primitive_20260127_150601`、`outputs/primitive_20260203_142150`、`outputs/primitive_20260204_100636`、`outputs/primitive_20260205_110416`。

**训练/验证**

| 版本 | Train | Val |
| --- | --- | --- |
| 0206 | 0.018559 | 0.034685 |
| 0205 | 0.051639 | 0.020851 |
| 0204 | 0.053487 | 0.022904 |
| 0203 | 0.053332 | 0.027751 |
| 0127 | 0.026071 | 0.027718 |
| 0118 | 0.024300 | 0.025395 |

**ID 一步预测**

| 数据集 | MVP（0118） | MVP（0127） | MVP（0203） | MVP（0204） | MVP（0205） | MVP（0206） | FNO（Single-Family） | FNO（Unified） | UPS-B | UPS-L |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| adv_beta0.4 | 0.010871 | 0.011057 | 0.011469 | 0.011150 | 0.013927 | 0.014731 | 0.011 | 0.0130 | 0.0027 | 0.0022 |
| burgers_nu0.001 | 0.044805 | 0.049449 | 0.036011 | 0.036329 | 0.034560 | 0.041926 | 0.042 | 0.0501 | 0.0399 | 0.0373 |
| diff_sorp | 0.002099 | 0.002506 | 0.004647 | 0.006824 | 0.003023 | 0.004046 | 0.0017 | 0.0041 | 0.0009 | 0.0009 |

**OOD-参数**

| 数据集 | MVP（0118） | MVP（0127） | MVP（0203） | MVP（0204） | MVP（0205） | MVP（0206） | UPS-B（0 samples） | FNO（0 samples） |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| adv_beta1.0 | 0.433994 | 0.406106 | 0.447640 | 0.330663 | 0.437249 | 0.393850 | — | — |
| burgers_nu1 | 0.256437 | 0.229572 | 0.300201 | 0.320175 | 0.398706 | 0.337694 | 0.0566 | 1.0342 |

**OOD-方程（reacdiff）**

| 数据集 | MVP（0118） | MVP（0127） | MVP（0203） | MVP（0204） | MVP（0205） | MVP（0206） | UPS-B（0 samples） | FNO（0 samples） |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| reacdiff_rho1 | 1.396651 | 1.909669 | 0.191827 | 0.260379 | 0.248967 | 0.139481 | 0.0557 | 0.1839 |
| reacdiff_rho5 | 0.544398 | 0.466353 | 0.118082 | 0.164833 | 0.200622 | 0.099158 | — | — |
| reacdiff_rho10 | 0.728883 | 0.640842 | 0.153330 | 0.155745 | 0.982652 | 0.136472 | — | — |

**路由使用（Primitive）**

注：以下统计来自 0118 的旧版路由；0127 启用 base 后会多一个 always-on 的 base 原语（索引为最后一个），未在此表中展示。

| 原语 | MVP（0118）选择次数 | MVP（0118）占比 |
| --- | --- | --- |
| P1 | 122710 | 17.0% |
| P2 | 167632 | 23.3% |
| P3 | 164587 | 22.9% |
